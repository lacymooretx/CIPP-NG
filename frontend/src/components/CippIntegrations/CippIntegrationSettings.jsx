...
const CippIntegrationSettings = ({ children }) => {
  ...
  // undefined -> real-data transition). Guarding on the payload itself rather than
  // `integrations.isSuccess` so an empty/stale cache entry can't reset the form to
  // nothing - `isSuccess` alone fired with `integrations.data` still undefined and
  // the form stayed on its per-Controller defaults.
  useEffect(() => {
