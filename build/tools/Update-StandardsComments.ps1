...
function Get-CIPPCapabilityPresets {
    ...
    $PresetAssignment = $LicenseFunction.Body.Find({
            ...
            $Node.Left.VariablePath.UserPath -eq 'Presets'
        }, $true)

    # '$Presets = @{...}' parses with the hashtable wrapped in a CommandExpressionAst/PipelineAst,
    # never bare on .Right - so search down for it instead of testing the assignment's type.
    $PresetHashtable = $PresetAssignment.Right.Find({
            param($Node)
            $Node -is [System.Management.Automation.Language.HashtableAst]
        }, $true)

    $Presets = @{}
    foreach ($Pair in $PresetHashtable.KeyValuePairs) {
        # @() is load-bearing - a single-element return unrolls to a string, and [0] would take its first character
...
$StandardsJSONPath = Join-Path $RepoRoot 'frontend\src\data\standards.json'
...
foreach ($Standard in $StandardsInfo) {
    ...
    # Join-Path, not Resolve-Path - the latter throws on a missing file, making the guard below unreachable
    $StandardsFilePath = Join-Path $BackendRoot "Modules\CIPPStandards\Public\Standards\$StandardFileName.ps1"
