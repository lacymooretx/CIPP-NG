...
1. **Must be at least Application Administration and User Administrator while setting up the integration.** These permissions may be removed after the integration has been set up.
...
2. **Microsoft MFA is mandatory.** Do not use alternative providers like Duo and ensure it is set up **before any login attempts.**
